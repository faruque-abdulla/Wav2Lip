...
async function main() {
  ...
  try {
    ...
    if (err instanceof SyncError) {
      console.error(
        `create generation request failed with status code ${err.statusCode} and error ${JSON.stringify(err.body)}`,
      );
    } else {
  ...
  while (status !== "COMPLETED" && status !== "FAILED") {
    ...
    try {
      ...
      if (err instanceof SyncError) {
        console.error(
          `polling failed with status code ${err.statusCode} and error ${JSON.stringify(err.body)}`,
        );
      } else {
  ...
  if (status === "COMPLETED") {
    console.log(
      `generation ${jobId} completed successfully, output url: ${generation?.outputUrl}`,
    );
  } else {
...
### Download required models for local use

Run the download helper in the repository root:
...
- `checkpoints/wav2lip_gan.pth` (Wav2Lip checkpoint)
- `face_detection/detection/sfd/s3fd.pth` (face detector model)

If you only want one file, use:

```bash
.venv\Scripts\python.exe download_models.py --checkpoint
.venv\Scripts\python.exe download_models.py --face
```

If the automatic download fails, download the checkpoint manually from the Google Drive links above and place it in `checkpoints/`.
...
A simple web UI is available in `webui.py`.

1. Download a Wav2Lip checkpoint and place it in `checkpoints/` or use a local path.
2. Download the face detector model with `python download_face_detection.py`.
3. Run:
...
The UI lets you upload a source face image/video and an audio file, then generates a lip-synced output video.
